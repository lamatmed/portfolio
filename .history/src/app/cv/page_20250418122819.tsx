        ...
        <Page size="A4" style={{ ...styles.page, padding: 20 }}>
            ...
            <View style={[styles.header, { marginBottom: 10 }]}>
                <View style={styles.headerText}>
                    <Text style={[styles.name, { fontSize: 22 }]}>{cvData.name}</Text>
                    <Text style={[styles.title, { fontSize: 11 }]}>{cvData.title}</Text>
                </View>
            ...
            <View style={{ marginBottom: 6 }}>
                <Text style={[styles.contactInfo, { fontSize: 9 }]}>{cvData.about}</Text>
            </View>
            ...
            <View style={styles.twoColumns}>
                ...
                <View style={styles.column}>
                    ...
                    <View style={[styles.section, { marginBottom: 8 }]}>
                        <Text style={[styles.sectionTitle, { fontSize: 13 }]}>Contact</Text>
                        {cvData.contacts.slice(0, 4).map((contact, index) => (
                            <View key={index} style={[styles.socialLink, { marginBottom: 2 }]}>
                                ...
                                <Text style={[styles.socialText, { fontSize: 8 }]}>{contact.text}</Text>
                            </View>
                        ))}
                    </View>
                    ...
                    <View style={styles.section}>
                        <Text style={[styles.sectionTitle, { fontSize: 13 }]}>Compétences</Text>
                        <Text style={[styles.itemSubtitle, { fontSize: 9 }]}>Techniques:</Text>
                        <View style={styles.skillsContainer}>
                            {cvData.technicalSkills.slice(0, 5).map((skill, index) => (
                                <Text key={index} style={[styles.skill, { fontSize: 8 }]}>{skill}</Text>
                            ))}
                        </View>
                        <Text style={[styles.itemSubtitle, { fontSize: 9, marginTop: 4 }]}>Soft skills:</Text>
                        <View style={styles.skillsContainer}>
                ...
                <View style={styles.column}>
                    ...
                    <View style={[styles.section, { marginBottom: 6 }]}>
                        <Text style={[styles.sectionTitle, { fontSize: 13 }]}>Expérience</Text>
                        {cvData.experience.slice(0, 2).map((exp, index) => (
                            <View key={index} style={{ marginBottom: 5 }}>
                                <Text style={[styles.itemTitle, { fontSize: 10 }]}>{exp.title}</Text>
                                <Text style={[styles.itemSubtitle, { fontSize: 8 }]}>{exp.company} • {exp.period}</Text>
                                <Text style={[styles.itemContent, { fontSize: 8 }]}>{exp.description}</Text>
                            </View>
                        ))}
                    </View>
                    ...
                    {/* Formation */}
                    <View style={styles.section}>
                        <Text style={[styles.sectionTitle, { fontSize: 13 }]}>Formation</Text>
                        {cvData.education.slice(0, 2).map((edu, index) => (
                            <View key={index} style={{ marginBottom: 5 }}>
                                <Text style={[styles.itemTitle, { fontSize: 10 }]}>{edu.title}</Text>
                                <Text style={[styles.itemSubtitle, { fontSize: 8 }]}>{edu.institution} • {edu.period}</Text>
                                <Text style={[styles.itemContent, { fontSize: 8 }]}>{edu.description}</Text>
                            </View>
                        ))}
                    </View>
                </View>
            ...
            <View style={[styles.section, { marginTop: 6 }]}>
                <Text style={[styles.sectionTitle, { fontSize: 13 }]}>Projets</Text>
