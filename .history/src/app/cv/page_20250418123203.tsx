        ...
        <Page size="A4" style={{ ...styles.page, padding: 30 }}>
            ...
            <View style={[styles.header, { marginBottom: 16 }]}>
                <View style={styles.headerText}>
                    <Text style={[styles.name, { fontSize: 24 }]}>{cvData.name}</Text>
                    <Text style={[styles.title, { fontSize: 13 }]}>{cvData.title}</Text>
                </View>
                <Image src="/l.jpg" style={{ ...styles.photo, width: 70, height: 70 }} />
            </View>

            {/* À propos */}
            <View style={{ marginBottom: 10 }}>
                <Text style={[styles.contactInfo, { fontSize: 10 }]}>{cvData.about}</Text>
            </View>
            ...
            <View style={styles.twoColumns}>
                ...
                <View style={styles.column}>
                    ...
                    <View style={[styles.section, { marginBottom: 10 }]}>
                        ...
                        {cvData.contacts.slice(0, 3).map((contact, index) => (
                            <View key={index} style={[styles.socialLink, { marginBottom: 3 }]}>
                                ...
                                <Text style={[styles.socialText, { fontSize: 9 }]}>{contact.text}</Text>
                            </View>
                        ))}
                    </View>
                    ...
                    <View style={styles.section}>
                        <Text style={[styles.sectionTitle, { fontSize: 14 }]}>Compétences</Text>
                ...
                <View style={styles.column}>
                    {/* Expérience */}
                    <View style={[styles.section, { marginBottom: 10 }]}>
                        <Text style={[styles.sectionTitle, { fontSize: 14 }]}>Expérience</Text>
                        {cvData.experience.slice(0, 2).map((exp, index) => (
                            <View key={index} style={{ marginBottom: 6 }}>
                                <Text style={[styles.itemTitle, { fontSize: 11 }]}>{exp.title}</Text>
                                <Text style={[styles.itemSubtitle, { fontSize: 9 }]}>{exp.company} • {exp.period}</Text>
                    ...
                    {/* Formation */}
                    <View style={styles.section}>
                        <Text style={[styles.sectionTitle, { fontSize: 14 }]}>Formation</Text>
                        {cvData.education.slice(0, 1).map((edu, index) => (
                            <View key={index} style={{ marginBottom: 6 }}>
                                <Text style={[styles.itemTitle, { fontSize: 11 }]}>{edu.title}</Text>
                                <Text style={[styles.itemSubtitle, { fontSize: 9 }]}>{edu.institution} • {edu.period}</Text>
            ...
            <View style={[styles.section, { marginTop: 12 }]}>
                <Text style={[styles.sectionTitle, { fontSize: 14 }]}>Projets</Text>
