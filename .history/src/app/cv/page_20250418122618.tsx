        ...
        <Page size="A4" style={{ ...styles.page, padding: 20 }}>
            ...
            <View style={[styles.header, { marginBottom: 10 }]}>
                <View style={styles.headerText}>
                    <Text style={[styles.name, { fontSize: 20 }]}>{cvData.name}</Text>
                    <Text style={[styles.title, { fontSize: 10 }]}>{cvData.title}</Text>
                </View>
                <Image src="/l.jpg" style={{ ...styles.photo, width: 60, height: 60 }} />
            </View>

            {/* À propos */}
            <View style={{ marginBottom: 6 }}>
                <Text style={[styles.contactInfo, { fontSize: 8 }]}>{cvData.about}</Text>
            </View>
            ...
            <View style={styles.twoColumns}>
                ...
                <View style={styles.column}>
                    ...
                    <View style={[styles.section, { marginBottom: 8 }]}>
                        <Text style={[styles.sectionTitle, { fontSize: 12 }]}>Contact</Text>
                        {cvData.contacts.slice(0, 4).map((contact, index) => (
                            <View key={index} style={[styles.socialLink, { marginBottom: 2 }]}>
                                <contact.icon style={styles.socialIcon} />
                                <Text style={[styles.socialText, { fontSize: 7 }]}>{contact.text}</Text>
                            </View>
                        ))}
                    </View>
                    ...
                    <View style={styles.section}>
                        <Text style={[styles.sectionTitle, { fontSize: 12 }]}>Compétences</Text>
                        <Text style={[styles.itemSubtitle, { fontSize: 8 }]}>Techniques:</Text>
                        <View style={styles.skillsContainer}>
                            {cvData.technicalSkills.slice(0, 5).map((skill, index) => (
                                <Text key={index} style={[styles.skill, { fontSize: 7 }]}>{skill}</Text>
                            ))}
                        </View>
                        <Text style={[styles.itemSubtitle, { fontSize: 8, marginTop: 4 }]}>Soft skills:</Text>
                        <View style={styles.skillsContainer}>
                ...
                <View style={styles.column}>
                    ...
                    <View style={[styles.section, { marginBottom: 6 }]}>
                        <Text style={[styles.sectionTitle, { fontSize: 12 }]}>Expérience</Text>
                        {cvData.experience.slice(0, 2).map((exp, index) => (
                            <View key={index} style={{ marginBottom: 5 }}>
                                <Text style={[styles.itemTitle, { fontSize: 9 }]}>{exp.title}</Text>
                                <Text style={[styles.itemSubtitle, { fontSize: 7 }]}>{exp.company} • {exp.period}</Text>
                                <Text style={[styles.itemContent, { fontSize: 7 }]}>{exp.description}</Text>
                            </View>
                        ))}
                    </View>
                    ...
                    {/* Formation */}
                    <View style={styles.section}>
                        <Text style={[styles.sectionTitle, { fontSize: 12 }]}>Formation</Text>
                        {cvData.education.slice(0, 2).map((edu, index) => (
                            <View key={index} style={{ marginBottom: 5 }}>
                                <Text style={[styles.itemTitle, { fontSize: 9 }]}>{edu.title}</Text>
                                <Text style={[styles.itemSubtitle, { fontSize: 7 }]}>{edu.institution} • {edu.period}</Text>
                                <Text style={[styles.itemContent, { fontSize: 7 }]}>{edu.description}</Text>
                            </View>
                        ))}
                    </View>
                </View>
            ...
            <View style={[styles.section, { marginTop: 6 }]}>
                <Text style={[styles.sectionTitle, { fontSize: 12 }]}>Projets</Text>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between' }}>
                    {cvData.projects.slice(0, 2).map((project, index) => (
                        <View key={index} style={{ width: '48%', marginBottom: 6 }}>
                            <Text style={[styles.itemTitle, { fontSize: 9 }]}>{project.title}</Text>
                            <Text style={[styles.itemContent, { fontSize: 7 }]}>{project.description}</Text>
                            <View style={styles.skillsContainer}>
                                {project.technologies.map((tech, i) => (
                                    <Text key={i} style={[styles.skill, { fontSize: 6 }]}>{tech}</Text>
                                ))}
                            </View>
                        </View>
                    ))}
