...
const Portfolio = () => {
  ...
  const handlePrint = useReactToPrint({
    pageContent: () => cvRef.current,
    documentTitle: "Lamat_Abdellahi_CV",
    pageStyle: `
      @page { size: A4; margin: 10mm; }
      @media print { body { -webkit-print-color-adjust: exact; } }
    `,
    onAfterPrint: () => alert("CV téléchargé!")
  });

  // OU Solution 2 (alternative typée)
  const printOptions: ReactToPrintProps = {
    content: () => cvRef.current,
    documentTitle: "Lamat_Abdellahi_CV"
  };
  const handlePrintAlt = useReactToPrint(printOptions);
