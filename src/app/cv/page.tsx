...
const MyDocument = () => (
    <Document>
        <Page size="A4" style={{ ...styles.page, padding: 35 }}>
            {/* En-tête */}
            <View style={[styles.header, { marginBottom: 20 }]}>
                <View style={styles.headerText}>
                    <Text style={[styles.name, { fontSize: 28 }]}>{cvData.name}</Text>
                    <Text style={[styles.title, { fontSize: 16 }]}>{cvData.title}</Text>
                </View>
                <Image src="/l.jpg" style={{ ...styles.photo, width: 90, height: 90 }} />
            </View>

            {/* À propos */}
            <View style={{ marginBottom: 14 }}>
                <Text style={[styles.contactInfo, { fontSize: 12, lineHeight: 1.6 }]}>
                    {cvData.about}
                </Text>
            </View>
            ...
            <View style={styles.twoColumns}>
                ...
                <View style={styles.column}>
                    ...
                    <View style={[styles.section, { marginBottom: 16 }]}>
                        <Text style={[styles.sectionTitle, { fontSize: 17 }]}>Contact</Text>
                        {cvData.contacts.slice(0, 3).map((contact, index) => (
                            <View key={index} style={[styles.socialLink, { marginBottom: 5 }]}>
                                <contact.icon style={styles.socialIcon} />
                                <Text style={[styles.socialText, { fontSize: 11 }]}>{contact.text}</Text>
                            </View>
                        ))}
                    </View>
                    ...
                    <View style={styles.section}>
                        <Text style={[styles.sectionTitle, { fontSize: 17 }]}>Compétences</Text>
                        <Text style={[styles.itemSubtitle, { fontSize: 11 }]}>Techniques :</Text>
                        <View style={styles.skillsContainer}>
                            {cvData.technicalSkills.slice(0, 5).map((skill, index) => (
                                <Text key={index} style={[styles.skill, { fontSize: 10 }]}>{skill}</Text>
                            ))}
                        </View>
                        <Text style={[styles.itemSubtitle, { fontSize: 11, marginTop: 5 }]}>Soft skills :</Text>
                        <View style={styles.skillsContainer}>
                            {cvData.softSkills.slice(0, 4).map((skill, index) => (
                                <Text key={index} style={[styles.skill, { fontSize: 10 }]}>{skill}</Text>
                            ))}
                        </View>
                    </View>
                ...
                <View style={styles.column}>
                    {/* Expérience */}
                    <View style={[styles.section, { marginBottom: 16 }]}>
                        <Text style={[styles.sectionTitle, { fontSize: 17 }]}>Expérience</Text>
                        {cvData.experience.slice(0, 2).map((exp, index) => (
                            <View key={index} style={{ marginBottom: 8 }}>
                                <Text style={[styles.itemTitle, { fontSize: 13 }]}>{exp.title}</Text>
                                <Text style={[styles.itemSubtitle, { fontSize: 11 }]}>{exp.company} • {exp.period}</Text>
                                <Text style={[styles.itemContent, { fontSize: 11, lineHeight: 1.5 }]}>{exp.description}</Text>
                            </View>
                        ))}
                    </View>
                    ...
                    {/* Formation */}
                    <View style={styles.section}>
                        <Text style={[styles.sectionTitle, { fontSize: 17 }]}>Formation</Text>
                        {cvData.education.slice(0, 1).map((edu, index) => (
                            <View key={index} style={{ marginBottom: 8 }}>
                                <Text style={[styles.itemTitle, { fontSize: 13 }]}>{edu.title}</Text>
                                <Text style={[styles.itemSubtitle, { fontSize: 11 }]}>{edu.institution} • {edu.period}</Text>
                                <Text style={[styles.itemContent, { fontSize: 11, lineHeight: 1.5 }]}>{edu.description}</Text>
                            </View>
                        ))}
                    </View>
                </View>
            ...
            <View style={[styles.section, { marginTop: 20 }]}>
                <Text style={[styles.sectionTitle, { fontSize: 17 }]}>Projets</Text>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between' }}>
                    {cvData.projects.slice(0, 2).map((project, index) => (
                        <View key={index} style={{ width: '48%', marginBottom: 10 }}>
                            <Text style={[styles.itemTitle, { fontSize: 12 }]}>{project.title}</Text>
                            <Text style={[styles.itemContent, { fontSize: 10 }]}>{project.description}</Text>
                            <View style={styles.skillsContainer}>
                                {project.technologies.map((tech, i) => (
                                    <Text key={i} style={[styles.skill, { fontSize: 9 }]}>{tech}</Text>
                                ))}
                            </View>
                        </View>
                    ))}
            ...
            <View style={[styles.section, { marginTop: 16 }]}>
                <Text style={[styles.sectionTitle, { fontSize: 17 }]}>Langues</Text>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
                    {[
                        { name: "Français", emoji: "🇫🇷" },
                        { name: "Arabe", emoji: "🇸🇦" },
                        { name: "Anglais", emoji: "🇬🇧" },
                        { name: "Portugais", emoji: "🇵🇹" },
                    ].map((lang, index) => (
                        <Text
                            key={index}
                            style={{
